...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.action === "verify_payment") {
      ...
      const { error } = await supabase.from("payments").update({ provider_payment_id: body.payment_id, status: "paid" }).eq("order_id", body.order_id).eq("user_id", user.id);
      if (error) return json({ error: "Could not save payment" }, 500);
