...
function App() {
  ...
  // Store click handler – navigates to filtered products with store products
  const openStore = (store: Store) => {
    if (store.product_ids && store.product_ids.length > 0) {
      setFilterConfig({ product_ids: store.product_ids });
      setFilterTitle(store.name);
      setScreen('filteredProducts');
    } else {
      // Fallback – go home
      setScreen('home');
    }
  ...
  if (screen === 'home') {
    ...
    content = <CategoriesScreen onCategory={openCategory} />;
  } else if (screen === 'orders') {
    content = <OrdersScreen onOrderClick={openOrder} />;
  } else if (screen === 'cart') {
    content = (
      <CartScreen
        cart={cart}
  ...
  } else if (screen === 'orderDetail' && selectedOrderId) {
    content = <OrderDetailScreen orderId={selectedOrderId} onBack={() => goTo('orders')} />;
  } else if (screen === 'addresses') {
    content = <AddressesScreen onBack={() => goTo('account')} onSaved={() => goTo('checkout')} />;
  } else if (screen === 'wishlist') {
    content = <WishlistScreen cart={cart} onProduct={openProduct} onShop={() => goTo('home')} />;
  } else if (screen === 'account') {
    content = <AccountScreen onNavigate={openProtected} />;
  ...
  } else if (screen === 'product' && selectedProductId) {
    content = (
      <ProductDetailScreen
  ...
  } else if (screen === 'filteredProducts' && filterConfig) {
    content = (
  ...
  } else if (screen === 'businessRegistration') {
    content = (
      <BusinessRegistrationScreen
        onBack={() => goTo('account')}
        onRegistered={handleBusinessRegistered}
  ...
  return (
    <div className="min-h-screen bg-ink-100">
      ...
      <div className="mx-auto min-h-screen max-w-[720px] bg-ink-50 shadow-2xl shadow-ink-200/50">
        ...
        <main className="py-4 pb-24 animate-fade-up">{content}</main>
        <BottomNavigation
          active={screen}
